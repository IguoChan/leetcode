...
func myAtoi(s string) int {
	symbol := 0     // 符号标记，未置为0，置-为1，置+为2
	hasNum := false // 是否读入了数字
	ret := 0
	for i := range s {
		// 前置空格，一旦出现数字或者正负号后空格不允许
		if s[i] == ' ' && !hasNum && symbol == 0 {
			continue
		}
		// 首次读入符号-
		if s[i] == '-' && !hasNum && symbol == 0 {
		...
		// 一旦读入非数字字符，且非前面情况，跳出循环
		if s[i] < '0' || s[i] > '9' {
			break
		}
		hasNum = true
		ret = ret*10 + int(s[i]-'0')
		// 在这里判断数字是否已经超过最大值，是的话就退出，这样比全部读取完再退出要好判断的多
